package com.github.xf717.systemservice.enums;
...
public interface SystemErrorCodeConstants {
  ...
  // ========== 权限模块 1002008000 ==========
  ErrorCode PERMISSION_DEMO_PERMISSION_DENY = new ErrorCode(1002008002,
      "演示账号，暂不允许写操作。欢迎加入我们的交流群：http://t.cn/EKEr5WE");
  ErrorCode PERMISSION_ROLE_ASSIGN_RESOURCE_NOT_EXISTS = new ErrorCode(1002008004,
      "分配角色资源时，有资源不存在");
  ...
  ErrorCode TWICE_PASSWORD_NOT_SAME = new ErrorCode(1002002001, "两次密码不相同");
  ErrorCode PASSWORD_NOT_NULL = new ErrorCode(1002003001, "密码不能为空");
  /**
   * 临时定义
   */
  ErrorCode NOT_EXISTS = new ErrorCode(1002003012, "不存在");


  ErrorCode INDUSTRY_TYPE_EXISTS = new ErrorCode(1002005101, "行业分类已存在");
  ErrorCode INDUSTRY_TYPE_NEXT_NODE_EXISTS = new ErrorCode(1002005102, "存在子分类，无法删除");
  ErrorCode INDUSTRY_TYPE_NOT_EXISTS = new ErrorCode(1002005103, "行业分类不存在");
  ErrorCode DICTIONARY_ITEM_NOT_EXISTS = new ErrorCode(1002005104, "字典值不存在");
