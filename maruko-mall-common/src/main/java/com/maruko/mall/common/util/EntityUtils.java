...
public class EntityUtils {
	 ...
	 * @param <T>
	 */
	public static <T> void setCreateAuditInfo(T entity) {
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes())
				.getRequest();
		String name = "";
		String id = "";
		...
		// 默认属性
		String[] fields = {"createTime", "createBy", "modifyTime", "modifyBy", "isDel"};
		Field field = ReflectionUtils.getAccessibleField(entity, "createTime");
		// 默认值
		Object[] value = null;
		if (field != null && field.getType().equals(Long.class)) {
			value = new Object[]{DateUtil.getCurrentTime(), StringUtils.isEmpty(id) ? 0 : Integer.valueOf(id), 0L, 0, 0};
		}
		// 填充默认属性值
		setDefaultValues(entity, fields, value);
	}
	 ...
	 * @param <T>
	 */
	public static <T> void setModifyAuditInfo(T entity) {
		HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes())
				.getRequest();
		String id = "";
		if (request != null) {
		...
		// 默认属性
		String[] fields = {"modifyTime", "modifyBy"};
		Field field = ReflectionUtils.getAccessibleField(entity, "modifyTime");
		Object[] value = null;
		if (field != null && field.getType().equals(Long.class)) {
			value = new Object[]{DateUtil.getCurrentTime(), StringUtils.isEmpty(id) ? 0 : Integer.valueOf(id) };
		}
		// 填充默认属性值
		setDefaultValues(entity, fields, value);
	}
