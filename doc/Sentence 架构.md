...
这两种方案呢，都有麻烦的地方，要不然就是所有服务都用一个domain类，OrderInfoDTO，要不就是各个服务对于订单相关的信息都有自己的定义，MyOrder，Order，OrderInfo，OrderInfoDTO。<!--我们现在用的就是第二种，各个服务自定义然后在进行对象转换,就是DTO可能会多些，但更灵活-->
...
重写一下接口的定义，使用map来传递json对象，各种参数尽可能扁平化的封装在一个map中，也可以的，或者是针对这个接口，专门定义一个特殊的domain类来接收。<!--map其实还是不太建议的-->
